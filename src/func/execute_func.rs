...
unsafe fn check_process(data: *mut func::FuncBlock, count: &mut i32) {
    let result = match_control_expr(data, *count);
    ...
    if result == -1 {
        ...
        *count = func::skip_next_len_indent(data, *count as c_int) as i32;
        ...
        func::assign_indent_value(get_target_indent_len(data, *count + 1).unwrap(), 4);
        *count = iterate_process(
        ...
        func::assign_indent_value(
            get_target_indent_len(data, *count + 1)
                .unwrap(),
            1);
    }
    *count += 1;
...
pub unsafe extern "C" fn execute_func(
    ...
    //  関数のデータを取得
        ...
        while run_number < (*data).process_length as i32 {
            check_process(data, &mut run_number);
        }
    } else {
        //  外部の関数を呼び出す
